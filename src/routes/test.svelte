...
<article class="prose flex-center">
			...
			{#each solnItems.filter((e) => e.visible) as item, i (item)}
				<div animate:flip class={item.classes} in:slide|local={{ duration: 700 }}>
					...
					{#if item['child']}
						...
						<div
							class="absolute text-center text-red-600 bg-blue-300/50 transition  duration-700 top-0 inset-x-0 mx-auto w-fit px-px text-black rounded-full"
	...
	<a class="px-4 my-2 self-end bg-orange-300" rel="prefetch" href="/substitution/exercise"> Try out an exercise >> </a>
</article>
